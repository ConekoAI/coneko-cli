...
function verifySignature(message, signatureB64, publicKeyB64) {
  try {
    ...
    const canonical = JSON.stringify(message, Object.keys(message).sort());
    return ed25519.verify(signature, Buffer.from(canonical), publicKey);
  } catch {
    return false;
  }
}
...
function encryptMessage(plaintext, recipientPublicB64) {
  ...
  // ECDH
  const sharedKey = x25519.getSharedSecret(ephemeralPrivate, recipientPublic);
...
function decryptMessage(encrypted, recipientPrivateB64) {
  ...
  // ECDH
  const sharedKey = x25519.getSharedSecret(recipientPrivate, ephemeralPublic);
